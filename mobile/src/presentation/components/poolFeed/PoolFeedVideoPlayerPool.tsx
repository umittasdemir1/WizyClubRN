/**
 * PoolFeedVideoPlayerPool - TikTok-style Video Player Recycling
 *
...
import { FEED_CONFIG, FEED_FLAGS } from './hooks/usePoolFeedConfig';
import { PoolFeedSlotRecycler, type PlayerSlot } from './utils/PoolFeedSlotRecycler';
import { PoolFeedVideoErrorHandler, ErrorAction } from './utils/PoolFeedVideoErrorHandler';
...
interface PoolFeedVideoPlayerPoolProps {
    videos: VideoEntity[];
    activeIndex: number;
    isMuted: boolean;
    isPaused: boolean;
...
export interface PoolFeedVideoPlayerPoolRef {
...
export const PoolFeedVideoPlayerPool = memo(forwardRef<PoolFeedVideoPlayerPoolRef, PoolFeedVideoPlayerPoolProps>(function PoolFeedVideoPlayerPool({
    videos,
    activeIndex,
    isMuted,
    isPaused,
    ...
    useEffect(() => {
        ...
        const recycleSlots = async () => {
            ...
            const { targetIndices, activeSlotIndex } = PoolFeedSlotRecycler.calculateTargetIndices(
                activeIndex,
                currentIdx,
                playableIndices,
                priorSlots,
    ...
    const handleError = useCallback(async (slotIndex: number, slotVideoId: string, error: any) => {
        ...
        const result = await PoolFeedVideoErrorHandler.handleError(slot, video, MAX_RETRIES, error);
