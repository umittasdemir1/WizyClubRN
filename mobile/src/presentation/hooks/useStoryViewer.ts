...
import { useStoryCacheStore } from '../store/useStoryCacheStore';

export function useStoryViewer(userId?: string) {
    const cachedStories = useStoryCacheStore((state) => state.stories);
    const setStoryCache = useStoryCacheStore((state) => state.setStories);
    ...
    const applyStories = useCallback((allStories: Story[]) => {
        // Group by user
        ...
        setIsLoading(false);
    }, [userId, viewedUserIds, initial]);

    useEffect(() => {
        if (cachedStories.length > 0) {
            applyStories(cachedStories);
        }
    }, [cachedStories, applyStories]);
    ...
    useEffect(() => {
        ...
        const fetchStories = async () => {
            ...
            if (!isMounted) return;
            setStoryCache(allStories);
            applyStories(allStories);
        };
        fetchStories();
        return () => { isMounted = false; };
    }, [userId, refreshTrigger, viewedUserIds, initial, currentUser?.id, setStoryCache, applyStories]);
