...
// Business thresholds for lifecycle classification
const STAGNANT_DAYS = 90; // Days since last sale → stagnant
const SLOW_DAYS = 45; // Days since last sale → slow
const STAGNANT_MULTIPLIER = 3; // Inventory ≥ 3× net sales → stagnant
const SLOW_MULTIPLIER = 1.5; // Inventory ≥ 1.5× net sales → slow
...
function computeLifecycleStatus(record, netSalesQty, daysSinceLastSale) {
    ...
    if ((daysSinceLastSale !== null && daysSinceLastSale > SLOW_DAYS) ||
...
export function analyzeInventory(records) {
    ...
    return {
        overview: {
            ...
            slowMovingItems: lifecycleCounts.get("Slow Moving") ?? 0,
            stagnantItems: lifecycleCounts.get("Stagnant") ?? 0
        },
        records: enriched,
