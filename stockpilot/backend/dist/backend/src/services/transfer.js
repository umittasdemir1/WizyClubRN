// Safety buffer: keep 1.5× net sales as target inventory before donating surplus
const SURPLUS_BUFFER_MULTIPLIER = 1.5;
export function buildTransferPlan(records) {
    ...
    for (const groupRecords of groups.values()) {
        const surplus = groupRecords
            ...
            const targetInventory = Math.max(Math.ceil(netSalesQty * SURPLUS_BUFFER_MULTIPLIER), 1);
            return {
                ...record,
        ...
        for (const receiver of deficit) {
            ...
            for (const donor of surplus) {
                if (donor.warehouseName === receiver.warehouseName ||
                    donor.available <= 0 ||
                    remainingNeed <= 0) {
                    continue;
                }
