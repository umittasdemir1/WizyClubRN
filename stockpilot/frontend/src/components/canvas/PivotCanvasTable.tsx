...
function PivotCanvasTableComponent({
    ...
    // Pre-compute field labels once — O(fields) instead of O(fields × .find()) per header cell
    const fieldLabelMap = useMemo(() => {
        const { colMap, metricMap } = buildFieldLookupMaps(view.columns, view.customMetrics);
        const map = new Map<string, string>();
        for (const fid of [...view.table.layout.rows, ...view.table.layout.columns, ...view.pivotResult.valueFields]) {
            if (!map.has(fid)) {
                map.set(fid, getFieldDefinitionFast(fid, colMap, metricMap, view.columnOverrides).label);
            }
        }
        return map;
    }, [view.columns, view.customMetrics, view.columnOverrides, view.table.layout.rows, view.table.layout.columns, view.pivotResult.valueFields]);
    ...
    return (
        ...
        >
            {view.filteredRecords.length > 0 && (
                <div className={`absolute inset-0 ${view.table.manualSize ? "overflow-auto" : "overflow-visible"}`}>
                        <table
                            ref={(node) => {
                                if (node) {
