...
import {
    buildFieldLookupMaps,
    formatAggregatedValueFast,
    resolveFieldValueFromAggregationStatesFast,
...
// ── View-level lookup map cache (avoids rebuilding per-call) ──
const _viewLookupCache = new WeakMap<PivotTableView, ReturnType<typeof buildFieldLookupMaps>>();
function getViewLookupMaps(view: PivotTableView) {
    let maps = _viewLookupCache.get(view);
    if (!maps) {
        maps = buildFieldLookupMaps(view.columns, view.customMetrics);
        _viewLookupCache.set(view, maps);
    }
    return maps;
}
...
function formatResolvedValue(
    ...
    const { colMap, metricMap } = getViewLookupMaps(view);
    return formatAggregatedValueFast(
        fieldId,
        resolveFieldValueFromAggregationStatesFast(fieldId, states, colMap, metricMap),
        colMap,
        metricMap,
        view.columnOverrides
    );
}
...
export function getValueFieldFilterOptions(view: PivotTableView, fieldId: PivotFieldId, headerFilterSortDirections: Record<string, HeaderFilterSortDirection>) {
    // Build lookup maps once outside the per-row loop (WeakMap cache still applies,
    // but avoids repeated property accesses on each renderCell → formatResolvedValue call).
    const { colMap, metricMap } = getViewLookupMaps(view);
    const options = Array.from(
        new Set(
            view.pivotResult.rowCombos.map((rowCombo) => {
                const rowBucket = view.pivotResult.matrix.get(rowCombo.key);
                const cell = rowBucket?.get("__total__");
                return formatAggregatedValueFast(
                    fieldId,
                    resolveFieldValueFromAggregationStatesFast(fieldId, cell, colMap, metricMap),
                    colMap,
                    metricMap,
                    view.columnOverrides
                );
...
export function getVisibleRowCombos(view: PivotTableView, headerFilterSelections: Record<string, string[]>, headerFilterSortDirections: Record<string, HeaderFilterSortDirection>) {
    ...
    // ── Pre-compute sort values ONCE to avoid renderCell inside O(n log n) comparator ──
    let valueSortMap: Map<string, string> | null = null;
    let valueSortDirection: HeaderFilterSortDirection | undefined;
    if (explicitValueSortFieldId) {
        valueSortDirection = getHeaderFilterSortDirection(
            view.table.id,
            "value-field",
            headerFilterSortDirections,
            explicitValueSortFieldId
        );
        valueSortMap = new Map();
        for (const combo of filteredRowCombos) {
            valueSortMap.set(combo.key, renderCell(view, combo.key, "__total__", explicitValueSortFieldId));
        }
    }

    const rowSortDirections = view.table.layout.rows.map((fieldId, rowIndex) =>
        getHeaderFilterSortDirection(view.table.id, "row-field", headerFilterSortDirections, fieldId, rowIndex)
    );

    return [...filteredRowCombos].sort((left, right) => {
        if (valueSortMap && valueSortDirection) {
            const comparison = compareSortableValues(
                valueSortMap.get(left.key)!,
                valueSortMap.get(right.key)!
            );
            if (comparison !== 0) {
                return sortByDirection(comparison, valueSortDirection);
            }
        }

        for (let rowIndex = 0; rowIndex < view.table.layout.rows.length; rowIndex++) {
            const comparison = compareSortableValues(left.labels[rowIndex], right.labels[rowIndex]);
            if (comparison !== 0) {
                return sortByDirection(comparison, rowSortDirections[rowIndex]);
            }
        }
