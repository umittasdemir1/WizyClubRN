...
function shouldUseLocalFallback(error: unknown) {
    return isAxiosError(error) && (error.code === "ERR_NETWORK" || error.code === "ECONNABORTED" || !error.response);
}
    ...
    return useMutation({
        mutationFn: async (file: File): Promise<UploadWorkflowResult> => {
            ...
            try {
                ...
                try {
                    const [{ parseInventoryFile }, { analyzeInventory, buildTransferPlan }] = await Promise.all([
