...
import { normalizeGenericRows } from "../utils/analysis";
import type { ParsedFileResult } from "../utils/analysis";
...
export async function parseInventoryFile(file: File): Promise<ParsedFileResult> {
    ...
    if (file.name.toLowerCase().endsWith(".csv")) {
        ...
        if (isConcatenatedQuotedCsv) {
            return normalizeGenericRows(parseConcatenatedQuotedCsv(text), file.name);
        }
    }
    ...
    return normalizeGenericRows(rows, file.name);
}
